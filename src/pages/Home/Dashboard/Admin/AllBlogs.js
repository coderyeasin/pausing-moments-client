...
const AllBlogs = () => {
    ...
    useEffect(() => {
        ...
        fetch(url)
            .then(res => res.json())
        .then(data => setBlogs(data.filter(e => e.location && e.info && e.cost).reverse()))
    }, [users?.email])
...
<div>
            ...
            <table className="table-auto md:w-full" >
                        ...
                        </thead>
                {
                    ...
                    blogs.map(event => <tbody key={event._id}>
                            ...
                            <td className="border-r-2 border-pink-900">{event.image.slice(0,10)}...</td>
                        ...
                        <button className='text-black bg-red-200 px-3'>Edit</button>
                            ...
                            <button onClick={()=> handleDelete(event._id)} className="text-red-400 px-3 bg-teal-100">X</button>
                            </td>
                            </tr>                      
                        </tbody>                      
)
